...
$(function() {
  ...
  ***************************/
     // Function to check if the site is in mobile mode (adjust the breakpoint as needed)
     function isMobileMode() {
      return window.innerWidth <= 767; // Change the breakpoint (767 is a common mobile breakpoint)
    }
    ...
    // Function to toggle the button visibility based on the current mode
    function toggleButtonVisibility() {
      if (isMobileMode()) {
        $('.sb-menu-btn').show(); // Show the button in mobile mode
        // Add click event listener to the button when in mobile mode
        $('.sb-menu-btn').on('click', handleMenuButtonClick);
      } else {
        $('.sb-menu-btn, .sb-navigation').removeClass('sb-active'); // Hide the button and navigation menu
        $('.sb-info-btn, .sb-info-bar, .sb-minicart').removeClass('sb-active');
        $('.sb-menu-btn').hide(); // Hide the button in PC mode
        // Remove the click event listener when not in mobile mode
        $('.sb-menu-btn').off('click', handleMenuButtonClick);
      }
    }
    ...
    // Add a window resize event listener to toggle the button visibility on resize
